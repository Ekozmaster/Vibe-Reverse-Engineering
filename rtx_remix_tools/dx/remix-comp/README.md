...
This version is adapted by [kim2091](https://github.com/kim2091) for use in the [Vibe Reverse Engineering](https://github.com/kim2091/Vibe-Reverse-Engineering) toolkit. It extends xoxor4d's remix-comp-base framework with:

- **FFP conversion system** — automatic shader-to-fixed-function pipeline conversion, ported from kim2091's standalone FFP proxy template. Captures VS constants, parses vertex declarations, and routes draw calls through FFP with configurable register layouts via INI.
- **Skinning module** — optional runtime-toggled vertex skinning with bone matrix upload, vertex buffer expansion, and compressed format decoding.
- **Diagnostic logging** — timed frame dump to `ffp_proxy.log` for debugging register layouts, vertex declarations, and draw call routing.
- **INI-based configuration** — all game-specific settings (register layout, albedo stage, skinning toggle) in `remix-comp.ini` instead of compile-time `#define`s.
- **ImGui FFP debug tab** — live VS constant heatmap, matrix viewer, texture stage bindings, and FFP enable/disable toggle.
- **Per-game build split** — shared static library stays in the base, only game-specific `comp/` code is copied per project.

<br>

##  Credits
- [xoxor4d](https://github.com/xoxor4d) - original remix-comp-base framework
- [kim2091](https://github.com/kim2091) - FFP conversion, skinning, diagnostics, and toolkit integration
